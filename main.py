...
def create_post(title: str, subtitle: str, authorid: int):
  ...
  # manipulação do banco de dados - pode dar erro
    try:
        post = Post(title=title, subtitle=subtitle, authorid=authorid)
        # adição do objeto instanciado
        session.add(post)
        # dar commit
        session.commit()
...
@app.put("/posts")
def put_post(id: int, title: str, subtitle: str, authorid: int):
    ...
    return JSONResponse(content={'id': post.id, 'title': post.title, 'subtitle': post.subtitle, 'idauthor': post.authorid}, status_code=200)
...
def get_all_posts():
    try:
        ...
        for post in post_list: # perguntar sobre created
            posts.append({'id':post.id, 'title': post.title, 'subtitle': post.subtitle, 'authorid': post.authorid})
    except Exception as e:
        session.rollback()
        return JSONResponse(content={'error': str(e)}, status_code=500)
...
def get_post(post_id: int):
    ...
    return JSONResponse(content={'id': post.id, 'title': post.title, 'subtitle': post.subtitle, 'authorid': post.authorid}, status_code=200)
...
def get_authorsposts(author_id: int):
    ...
    try:
        ...
        posts = []
        for post in post_list: # perguntar sobre created
            posts.append({'id':post.id, 'title': post.title, 'subtitle': post.subtitle})
    except Exception as e:
        session.rollback()
        return JSONResponse(content={'error': str(e)}, status_code=500)
